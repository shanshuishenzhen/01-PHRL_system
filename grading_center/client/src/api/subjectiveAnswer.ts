...
// 获取待阅卷答案列表
export const getPendingAnswers = async (examId: number, page = 1, limit = 10) => {
  const response = await apiClient.get(`/subjective-answers/pending`, {
...
// 获取评阅中的答案列表
export const getMarkingAnswers = async (examId: number, page = 1, limit = 10) => {
  const response = await apiClient.get(`/subjective-answers/marking`, {
    params: { examId, page, limit }
  });
  return response.data;
};
...
export const getDisputedAnswers = async (examId: number, page = 1, limit = 10): Promise<PaginatedResponse<SubjectiveAnswer>> => {
  const response = await apiClient.get(`/subjective-answers/disputed`, {
    params: { examId, page, limit }
  });
  return response.data;
};
...
export const disputeAnswer = async (id: number, reason: string): Promise<{ message: string }> => {
  const response = await apiClient.post(`/subjective-answers/${id}/dispute`, { reason });
  return response.data;
};
...
export const arbitrateAnswer = async (id: number, adjustedScore: number, resolution: string): Promise<{ message: string; data: SubjectiveAnswer }> => {
  const response = await apiClient.post(`/subjective-answers/${id}/arbitrate`, { adjustedScore, resolution });
  return response.data;
};